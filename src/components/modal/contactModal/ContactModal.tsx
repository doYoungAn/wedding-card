  ...
  return (
    <div className='fixed top-0 left-0 w-[100vw] h-[100vh] bg-modalDarkBackBg flex flex-col z-[1000] overflow-auto'>
      <div className='w-full h-[80px] flex items-center justify-end px-4'>
        <CloseIcon width={30} height={30} className='fill-[#fff]' onClick={() => onClickClose?.()} />
      </div>
        ...
        <div className='mb-8'>
          ...
          {GROOM_PEOPLE.map((person) => (
            <div className='flex flex-row py-4 font-IropkeBatangM'>
              <div className='flex-1 flex items-center text-[#ccc] text-sm'>{person.description}</div>
              <div className='flex-1 text-[#fff] font-bold text-center'>{person.name}</div>
              <div className='flex-1 flex items-center justify-center gap-4'>
                <PhoneIcon className='fill-[#ccc]' width={20} height={20} onClick={() => onClickPhone(person.phone)} />
                <MessageIcon className='fill-[#ccc]' width={20} height={20} onClick={() => onClickMessage(person.phone)} />
              </div>
            </div>
          ))}
        </div>
